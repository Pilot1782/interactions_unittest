...
class TestCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_example_slash(self):
        ...
        self.assertTrue(
            actions[0].action_type == ActionType.DEFER, f"Action 0: {actions[0]}"
        )
        self.assertTrue(
            actions[1].action_type == ActionType.SEND, f"Action 1: {actions[1]}"
        )
        self.assertTrue(
            actions[2].action_type == ActionType.EDIT, f"Action 2: {actions[2]}"
        )
        self.assertTrue(
            actions[3].action_type == ActionType.DELETE, f"Action 3: {actions[3]}"
        )
    ...
    async def test_list_category_slash(self):
        """Test the list channel slash command."""
        client = get_client()
        fake_guild = FakeGuild(
            client=client,
            channel_names={
                "welcome": [],
                "smalltalk": ["chan_a", "chan_b"],
                "general": ["chan_c", "chan_d"],
            },
            role_names=[],
            member_names={},
        )
        ...
        self.assertTrue(
            "Categories" in actions[1].message["content"],
            f"Action 1: {actions[1].message['content']}",
        )
        self.assertTrue(
            len(actions[1].message["content"].split("\n")) == 8,
