...
from commands import test_slash, MyExtension, list_channel_slash, list_role_slash, list_member_slash
...
class TestCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_channel_slash(self):
        fake_guild = FakeGuild(channel_names={"welcome": [],"smalltalk": [],"general": []})
        ...
        self.assertTrue(actions[0]["action"] == "defer", f'Action 0: {actions[0]}')
        self.assertTrue(actions[1]["action"] == "send", f'Action 1: {actions[1]}')
        self.assertTrue("Channels" in actions[1]["message"]["content"], f'Action 1: {actions[1]}')
        self.assertTrue(len(actions[1]["message"]["content"].split('\n')) == 4, f'Action 1: {actions[1]["message"]["content"]}')

    async def test_list_role_slash(self):
        fake_guild = FakeGuild(channel_names={"welcome": [],"smalltalk": [],"general": []},role_names=["admin","mod","user"])
    ...
    async def test_list_member_slash(self):
        fake_guild = FakeGuild(channel_names={"welcome": [],"smalltalk": [],"general": []},role_names=["admin","mod","user"],member_names={"user1": ["user","admin"],"user2": ["user"],"user3": ["user"]})
        ...
        self.assertTrue(actions[0]["action"] == "defer", f'Action 0: {actions[0]}')
        self.assertTrue(actions[1]["action"] == "send", f'Action 1: {actions[1]}')
        self.assertTrue("Members" in actions[1]["message"]["content"], f'Action 1: {actions[1]}')
        self.assertTrue(len(actions[1]["message"]["content"].split('\n')) == 4, f'Action 1: {actions[1]["message"]["content"]}')
