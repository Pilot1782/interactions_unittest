...
class FakeGuild(Guild):
    ...
    def __init__(self, channel_names:dict[str,list[str]],role_names:list[str]=None,member_names:dict[str,list[str]]=None, *args, **kwargs):
        client = get_client()
        super().__init__( client = client, id=random_snowflake(), name="VirtualTest", preferred_locale='english_us', owner_id=random_snowflake(),*args, **kwargs)
        for channel,sub_channels in channel_names.items():
            channel_id = random_snowflake()
            if not sub_channels:
                self.channels.append(FakeChannel(client = client, name=channel, id=channel_id))
            else:
                ...
                for sub_channel_name in sub_channels:
                    sub_channel = FakeChannel(client = client, name=sub_channel_name, id=random_snowflake(), parent_id=category.id)
                    self.channels.append(sub_channel)
                    category.channels.append(sub_channel)
        self.roles.extend(FakeRole(name=role,client = client, color='#ffffff', position=order, guild_id=self.id, permissions=Permissions.ALL,  id=random_snowflake()) for order,role in enumerate(role_names) or [])
        self.members.extend(
            FakeMember(nick=member, id=random_snowflake(),fake_roles=[role for role in self.roles if role.name in role_names ],guild_id=self.id,client = client)
...
async def call_slash(func: typing.Callable, *args, _client: FakeClient = None, **kwargs):
    ...
    for key, value in kwargs.items():
        if key.startswith('test_ctx_'):
            ctx.__setattr__(key.split('test_ctx_',1)[1], value)

    ctx.args = args
    ctx.kwargs = {key: value for key, value in kwargs.items() if not key.startswith('text_ctx')}
